...
class BERT(nn.Module):
    def __init__(self):
        ...
        in_channels = 1
        out_channels = 100
        kernel_heights=[2,4,6]

        #torch.nn.init.xavier_normal_(self.classifier.weight)
        self.conv1 = nn.Conv2d(in_channels, out_channels, (kernel_heights[0], self.hidden_size))
        self.conv2 = nn.Conv2d(in_channels, out_channels, (kernel_heights[1], self.hidden_size))
        self.conv3 = nn.Conv2d(in_channels, out_channels, (kernel_heights[2], self.hidden_size))
        self.label = nn.Linear(len(kernel_heights)*out_channels, 2)
    ...
    def forward(self,label,inputs,attention_mask):
        ...
        print('len:last_hidden_state',len(last_hidden_state))
        #LSTM
        ...
        activation1 = F.relu(self.conv1(layer)).squeeze(3)
        max_out1 = F.max_pool1d(activation1, activation1.size()[2]).squeeze(2)
    
        activation2 = F.relu(self.conv2(layer)).squeeze(3)
        max_out2 = F.max_pool1d(activation2, activation2.size()[2]).squeeze(2)
        
        activation3 = F.relu(self.conv3(layer)).squeeze(3)
        max_out3 = F.max_pool1d(activation3, activation3.size()[2]).squeeze(2)

        all_out = torch.cat((max_out1, max_out2, max_out3), 1)
        

        output = self.dropout(all_out)
    ...
    #print('acc',epoch_acc / len(train_loader))
            ...
            if global_step % eval_every == 0:
                ...
                if best_valid_loss > average_valid_loss:
                    ...
                    save_checkpoint(file_path + '/' + 'model_b_cnn.pt', model, best_valid_loss)
                    save_metrics(file_path + '/' + 'metrics_b_cnn.pt', train_loss_list, valid_loss_list, global_steps_list)
...
load_checkpoint(file_path + '/model_b_cnn.pt', best_model)
